...
def pca(X=np.array([]), no_dims=50):
    """Runs PCA on the NxD array X to reduce its dimensionality to no_dims dimensions."""
...
def request_(req_url, auth_token, sleep_time=1):
    headers = {
...
def main():
    ...
    auth_token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJTdWJqZWN0IjoiN2Q1ZTQ5NjQtNTUyMC00MTUxLTlkYWYtMWM4YmNlZmQxY2YyIiwiU3RlYW1JZCI6IjExMzE0OTgzMTAiLCJuYmYiOjE2OTI4NDcxNTMsImV4cCI6MTcyNDM4MzE1MywiaWF0IjoxNjkyODQ3MTUzLCJpc3MiOiJodHRwczovL2FwaS5zdHJhdHouY29tIn0.hy_J0sxfdHDq6VBJkv6i1wlU5gry1L-_TjeAjg3hpvI'  # replace with your actual auth token
...
match(ids: [4986461644]) {
  stats {
   pickBans {
    ...
    outputs = []
    info = json.load(
        request_("https://api.stratz.com/api/v1/hero/directory/detail", auth_token))
    ...
    for hero in info["heroes"]:
        ...
        item_data = json.load(request_(
            "https://api.stratz.com/api/v1/hero/%s/purchasePattern" % hero["heroId"], auth_token))
        items = np.zeros(265)
    ...
    def plot(Y):
        ...
        for i in range(113):
            ...
            ab = AnnotationBbox(
                imagebox, (Y[i, 0], Y[i, 1]), frameon=False, xycoords='data')
            ax.add_artist(ab)
